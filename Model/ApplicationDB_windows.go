...
func GetApplicationList_WMI() []DapplicationDB {
	...
	uuid := sysuil.GetUniqueID()

	var applist []DapplicationDB
	var win32List []Win32_Product
	...
	for i := range win32List {
		...
		win32List[i].PackageCode = input

		applist = append(applist, DapplicationDB{
			0,
			uuid,
			win32List[i].Name,
			win32List[i].Version,
			win32List[i].Language,
			win32List[i].Vendor,
			win32List[i].InstallDate2,
			win32List[i].InstallLocation,
			win32List[i].InstallSource,
			win32List[i].PackageName,
			win32List[i].PackageCode,
			win32List[i].RegCompany,
			win32List[i].RegOwner,
			win32List[i].URLInfoAbout,
			win32List[i].Description,
			false,
			time.Now(),
			time.Now(),
			time.Now(),
